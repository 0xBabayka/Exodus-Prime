...
20, wear: 0, loc: 'inventory' },
        ...
        waterSlots: [{id:0},{id:1}], 
            ...
            greenhouse: { slots: Array(6).fill(null).map((_, i) => ({ id: 
i, status: 'empty' })) },
...
machineParts: { slots: [{id:0},{id:1},{id:2}] },
            ...
            skills: {
                ...
                agriculture: { lvl: 1 }, metallurgy: { 
lvl: 1 }, chemistry: { lvl: 1 }, 
...
app.get('/api/game/load', auth, async (req, res) => {
    try {
        ...
        responseState.lastSaveTime 
= user.lastSaveTime ? new Date(user.lastSaveTime).getTime() : Date.now();
...
let timeSinceLastSave = 0;
        ...
        if (oldState.inventory && typeof oldState.inventory.get === 'function') {
            dbHelium3 = oldState.inventory.get('Helium3') ||
0;
        } else if (oldState.inventory) {
            dbHelium3 = oldState.inventory.Helium3 ||
0;
        }
...
if (timeSinceLastSave < 1000) { 
        ...
        if (newState.scavenging?.active && !oldState.scavenging?.active) {
            instantPowerSpent += 1;
        }
        ...
        let remainingCharge = Math.max(0, expectedNewCharge);
        if (newState.power && Array.isArray(newState.power.batteries)) {
            newState.power.batteries.forEach(newBat => {
                ...
                if (newBat.loc === 'grid') {
                    ...
                    if (remainingCharge > capacity) {
                        newBat.charge = capacity;
                        remainingCharge -= capacity;
                    } else {
                        newBat.charge = remainingCharge;
                        remainingCharge = 0;
                    }
                } else {
                    newBat.charge = oldBat ? (oldBat.charge || 0) : 0;
                }
            });
            
            newState.power.gridStatus = (newGridBats.length > 0 && expectedNewCharge > 0.01) ? "ONLINE" : "BLACKOUT";
            if (expectedNewCharge <= 0 && generatedPower < totalPowerSpent) {
...
return res.status(400).json({ msg: 'Game integrity error: Stamina values exceed allowable limits.' });
            ...
            if (oldState.stamina && oldState.inventory && newState.inventory) {
                const oldVal = oldState.stamina ?
parseFloat(oldState.stamina.val) || 0 : 0;
...
if (oldState.cooldowns) {
                for (const item of TRACKED_FOODS) {
                    if (typeof oldState.cooldowns.get === 'function') {
                        newState.cooldowns[item] = oldState.cooldowns.get(item) ||
0;
                    } else {
                        newState.cooldowns[item] = oldState.cooldowns[item] ||
0;
                    }
                }
            }
...
return res.status(400).json({ msg: `Game integrity error: Cannot consume multiple ${foodItem} at once.` });
                    ...
                    const lastEaten = newState.cooldowns[foodItem] ||
0;
        ...
        if (now - lastClaim < DAILY_COOLDOWN) 
{
        ...
        currentEnergyBars = user.gameState.inventory.get("Energy Bar") || 0;
            user.gameState.inventory.set("Energy Bar", currentEnergyBars + 1);
             ...
             currentEnergyBars = user.gameState.inventory["Energy Bar"] ||
0;
...
app.post('/api/market/offer', auth, async (req, res) => {
    ...
    try {
        ...
        if (!safeItem || qty < 1 || 
price < 1 || !Number.isInteger(qty) || !Number.isInteger(price)) {
             await session.abortTransaction();
    ...
    try {
       ...
       if(!offer) {
        ...
        if(typeof user.gameState.inventory.get === 'function') {
            const current = user.gameState.inventory.get(offer.item) ||
0;
            user.gameState.inventory.set(offer.item, current + offer.qty);
        } else {
            const current = user.gameState.inventory[offer.item] ||
0;
...
return res.status(403).json({ msg: "Anti-Cheat: Device Fingerprint Match. Cannot trade between accounts on the same device." });
        ...
        const buyerH3 = typeof buyer.gameState.inventory.get === 'function' ? buyer.gameState.inventory.get('Helium3') : buyer.gameState.inventory['Helium3'] ||
0;
...
if(typeof seller.gameState.inventory.set === 'function') {
                const currentSellerH3 = seller.gameState.inventory.get('Helium3') ||
0;
                seller.gameState.inventory.set('Helium3', currentSellerH3 + sellerRevenue);
            } else {
                const currentSellerH3 = seller.gameState.inventory['Helium3'] ||
0;
