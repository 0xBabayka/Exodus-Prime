...
= jwt.verify(token, JWT_SECRET);
    ...
    message: { msg: 'Market transaction limit reached for your account. 
Slow down.' }
});
...
const MAX_ALLOWED_CHARGE = BATTERY_CAP + 2.0; 
            // ИСПРАВЛЕНИЕ: Точная валидация на основе максимальной выдачи панелей
...
const chargeDelta = totalNewCharge - totalOldCharge;
                ...
                // ИСПРАВЛЕНИЕ: Увеличен допуск (буфер) для компенсации рассинхронизации между 
                // requestAnimationFrame (который замирает в неактивной вкладке клиента) 
                // и setInterval (который продолжает сохранять игру).
                // Буфер 100.0 покрывает максимальную емкость грида, предотвращая ложные баны.
                const maxPossibleGain = (elapsedSeconds * MAX_GEN_PER_SEC) + 100.0;
